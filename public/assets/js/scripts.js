...
$(function () {
  ...
  $("#sticky_header_setting").on("change", function () {
    ...
    $.ajax({
      url: `${base_path}/sticky`,
      type: 'post',
      data: {_token: CSRF_TOKEN,sticky:stk}
     ...
     if(actualizado == 1){
      if (stk == "1") {
        iziToast.success({
          title: 'Actualizado!',
          message: 'Sticky Topbar activado..',
          position: 'topRight'
        });
      }else{
        iziToast.success({
          title: 'Actualizado!',
          message: 'Sticky Topbar desactivado..',
          position: 'topRight'
        });
      }
  ...
  $(".choose-theme li").on("click", function () {
    ...
    var colorAux = $(this).attr("title");
    $.ajax({
      url: `${base_path}/color_teme`,
      type: 'post',
      data: {_token: CSRF_TOKEN,color:colorAux}
    }).done(function( actualizado ) {
  ...
  $(".sidebar-color input:radio").change(function () {
    ...
    $.ajax({
      url: `${base_path}/side_teme`,
      type: 'post',
      data: {_token: CSRF_TOKEN,tema:color}
    }).done(function( actualizado ) {
     ...
     if(actualizado == 1){
      if (tema == "1") {
        iziToast.success({
          title: 'Actualizado!',
          message: 'Sidebar Claro seleccionado..',
          position: 'topRight'
        });
      }else{
        iziToast.success({
          title: 'Actualizado!',
          message: 'Sidebar Oscuro seleccionado..',
          position: 'topRight'
        });
      }
  ...
  $(".btn-restore-theme").on("click", function () {
    ...
    $.ajax({
      url: `${base_path}/restaurar_pc`,
      type: 'get',
      data: {_token: CSRF_TOKEN}
    }).done(function( restaurado ) {
     if(restaurado == 1){
      iziToast.success({
        title: 'Actualizado!',
        message: 'Se reustauro el panel de configuraciones..',
        position: 'topRight'
      });
     }else{
      iziToast.error({
  ...
  //start up class add
  let color_fondo = $('#cp_color_fondo').val();
  let color_sidebar = $('#cp_color_sidebar').val();
  let color_tema = $('#cp_color_tema').val();
  let mini_sidebar = $('#cp_mini_sidebar').val();
  let sticky_topbar = $('#cp_sticky_topbar').val();
  ...
  jQuery("body").addClass("theme-"+color_tema);
  
  // set theme default color
  $(".choose-theme li").removeClass("active");
  $(".choose-theme li[title|='"+color_tema+"']").addClass("active");
  //set default dark or light layout(1=light, 2=dark)
  $(".select-layout[value|='"+color_fondo+"']").prop("checked", true);
  //set default dark or light sidebar(1=light, 2=dark)
  $(".select-sidebar[value|='"+mini_sidebar+"']").prop("checked", true);
